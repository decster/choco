...
public:
	...
	bool endsWith(const Slice& s) const;
//	int64_t toInt(int64_t default_value=0) const;
//	double toDouble(double default_value=0) const;
	Slice trim() const;
	std::vector<Slice> split(const char sep, bool trim = true) const;

	static string join(const std::vector<Slice>& strs, const Slice& sep);
